...
class Pedestrian (Supervisor):
    ...
    def __init__(self):
        ...
        self.speed = 2.0
        # current lin and rot velocities
    ...
    def update_pose(self, timestep: float) -> None:
        ...
        self.pose.x += math.cos(self.pose.theta) * \
            self.current_twist.linear.x * timestep
        self.pose.y += math.sin(self.pose.theta) * \
            self.current_twist.linear.x * timestep
    ...
    def update_joints(self, timestep: float) -> None:
        ...
        self.walk_cycle_progress = (
            self.walk_cycle_progress + 2*x_delta * self.CYCLE_TO_DISTANCE_RATIO) % 1
        ...
        h2 = self.height_offsets[(current_sequence + 1) %
                                 self.WALK_SEQUENCES_NUMBER]
    ...
    def keyboard_control(self):
        ...
        while key in ["up", "down", "left", "right"]:
            if key == "up":
                self.current_twist.linear.x = self.speed*2
                # print(self.speed)
            elif key == "down":
                self.current_twist.linear.x = -self.speed*2
            elif key == "right":
                self.current_twist.angular.z = -self.speed*2
            elif key == "left":
                self.current_twist.angular.z = self.speed*2
...
if __name__ == '__main__':
    print("\n")
    print("====== Controls =======\n\n")
    print(" The person can be controlled from your keyboard!\n")
    print(" All controllable movement is in body coordinates\n")
    print("- Use the up and down keys to move forawrd and backwards\n")
    print("- Use left and right to rotate around yaw\n ")
